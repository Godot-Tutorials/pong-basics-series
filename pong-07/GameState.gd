...
onready var screenBox: BoundBox07 = BoundBox07.new(screen)
# object instancing
onready var ball: Ball07 = Ball07.new(screenBox.getCenter())
onready var playerPaddle: PlayerPaddle07 = PlayerPaddle07.new(screenBox)
onready var aiPaddle: AiPaddle07 = AiPaddle07.new(screenBox)
onready var instructionText: Text07 = Text07.new(
	"Start a game by pressing the spacebar",
	Vector2(screenBox.getSize().x/2.0, 0.0)
)

onready var playerScoreText: Text07 = Text07.new(
	playerScore as String,
	Vector2(screenBox.getSize().x/4.0, _SCORE_HEIGHT_PADDING) # 25% from the left
)

onready var aiScoreText: Text07 = Text07.new(
	aiScore as String,
...
func checkCollisions() -> void:
	...
	if Collisions07.pointToRectangle(ball.getPosition(), playerPaddle.getRect()) and ball.isMovingLeft():
		ball.inverseXSpeed()
	
	if Collisions07.pointToRectangle(ball.getPosition(), aiPaddle.getRect()) and ball.isMovingRight():
		ball.inverseXSpeed()
