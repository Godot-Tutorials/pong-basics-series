...
onready var screenBox: BoundBox09 = BoundBox09.new(screen)
# object instancing
onready var ball: Ball09 = Ball09.new(screenBox.getCenter())
onready var playerPaddle: PlayerPaddle09 = PlayerPaddle09.new(screenBox)
onready var aiPaddle: AiPaddle09 = AiPaddle09.new(screenBox)
onready var instructionText: Text09 = Text09.new(
	"Start a game by pressing the spacebar",
	Vector2(screenBox.getSize().x/2.0, 0.0)
)

onready var playerScoreText: Text09 = Text09.new(
	playerScore as String,
	Vector2(screenBox.getSize().x/4.0, _SCORE_HEIGHT_PADDING) # 25% from the left
)

onready var aiScoreText: Text09 = Text09.new(
	aiScore as String,
...
func checkCollisions() -> void:
	...
	if Collisions09.pointToRectangle(ball.getPosition(), playerPaddle.getRect()) and ball.isMovingLeft():
		playerPaddle.changeBallDirection(ball)
	
	if Collisions09.pointToRectangle(ball.getPosition(), aiPaddle.getRect()) and ball.isMovingRight():
