...
func changeBallDirection(ball: Ball11) -> void:
	var ballY = ball.getPosition().y
	
	var magnitude: float = Math11.pointConversion(ballY, _pos.y, _pos.y+_size.y, maxMagnitude, -maxMagnitude)
	var degree: float = Math11.pointConversion(ballY, _pos.y, _pos.y+_size.y, maxRotation, -maxRotation)
