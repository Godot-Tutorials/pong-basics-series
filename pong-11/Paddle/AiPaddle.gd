...
func changeChasePosition() -> void:
	var rndDistribution: float = Math11.primitiveNormalDistributionRandom()
	var rndMin: float = 0.0
	var rndMax: float = 1.0
	var minChaseSize: float = 0.0
	var maxChaseSize: float = _size.y
	
	chasePosition = Math11.pointConversion(rndDistribution, rndMin, rndMax, minChaseSize, maxChaseSize)
