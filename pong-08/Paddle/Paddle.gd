...
func changeBallDirection(ball: Ball08) -> void:
	var ballY = ball.getPosition().y
	
	var magnitude: float = Math08.pointConversion(ballY, _pos.y, _pos.y+_size.y, maxMagnitude, -maxMagnitude)
	var degree: float = Math08.pointConversion(ballY, _pos.y, _pos.y+_size.y, maxRotation, -maxRotation)
